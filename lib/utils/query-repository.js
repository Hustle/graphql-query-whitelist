import parseQuery, { parseASTQuery } from './parse-query'
...
class QueryRepository {
  ...
  async putAST(queryAST, options = {}) {
    let { queryId, operationName, normalizedQuery } = parseASTQuery(queryAST, { requireOperationName: false });
    const queryObj = { query: normalizedQuery, operationName, enabled: true }
    await this.store.set(queryId, queryObj)
